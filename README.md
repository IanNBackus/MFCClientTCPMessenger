...
Async I/O via the CAsyncSocket class is accomplished via events, which requires implementing a number of events and event handlers.

The best practice of CAsyncSocket class is to create a subclass that inherits CAsyncSocket. This is due to the fact that we must create user-defined windows messages to map our event handlers to (a quirk of how custom events, or should I say "messages", work in MFC). In our example "SocketController" is the subclass in question.
